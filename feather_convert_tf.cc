#include "tensorflow/core/framework/grapdh.pb.h"
...
{
    int fd = open(pb_model.c_str(), O_RDONLY);
    if (fd < 0)
    {
        std::cerr << "read caffe model prototxt " << pb_model  <<" fail!" << std::endl;
        return false;
    }

    FileInputStream* input = new FileInputStream(fd);
    bool success = google::protobuf::TextFormat::Parse(input, &pb_graph);
    delete input;
    close(fd);
    return success;
}
